...
export async function Rooms({ searchParams }: { searchParams?: SearchParamsType }) {
    ...
    return (
        <div className="flex flex-col items-center gap-4 w-full">
            {rooms?.length ? (
                rooms.map((room) => (
                    <Link href={`/rooms/${room.id}`} key={room.id} className="flex flex-col md:flex-row gap-2 md:gap-8 bg-purple-3/10 dark:bg-dark-3 border dark:border-dark-2 border-purple-3/5 rounded-lg p-4 md:w-full max-w-6xl">
                        ...
                        <div className="flex flex-col gap-4 justify-between p">
                            <div className="flex flex-col">
                                ...
                                <span><span className="font-medium text-base">Price: </span>{`Rs.${room.pricePerHour}/hour, Rs.${room.pricePerDay}/day, Rs.${room.pricePerMonth}/month`}</span>
                                <span><span className="font-medium text-base">Size: </span>{room.lengthInFeet}×{room.widthInFeet} square feet</span>
                                <span><span className="font-medium text-base">Address: </span>{room.address} {room.city}</span>
                            </div>
